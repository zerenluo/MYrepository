...
class Hypothesis(object):
    ...
    # def RodvecandTrans(cls, rodandtrans = np.zeros(6)):
    #     rodvec = rodandtrans[:3, 1]
    #     length = np.linalg.norm(rodvec)
    #     trans = rodandtrans[3:6, 1]
    #     if length > 1e-5:
    #         rot = cv2.Rodrigues(rodvec)
    #         return cls(rot, trans, inv(rot))
    #     else:
    #         rot = np.eye(3)
    #         return cls(rot, trans, inv(rot))
    ...
    def invTransform(self, point = np.zeros(3)):
        point -= self.translation
        tp = np.dot(self.invRotation, point)
        return tp
    ...
    def getInv(self):
        ...
        return h
        # h = Hypothesis.TransformationMatrix(Hypothesis, inv(self.getTransformation()))
        # return h
    ...
    def __mul__(self, other):
        ...
        return h
    # def __mul__(self, other):
    #     h = Hypothesis.TransformationMatrix(Hypothesis, np.dot(self.getTransformation(), other.getTransformation()))
    #     return h
    ...
    def __truediv__(self, other):
        ...
        return h
    # def __truediv__(self, other):
    #     h = Hypothesis.TransformationMatrix(Hypothesis, np.dot(self.getTransformation(), inv(other.getTransformation())))
    #     return h

    def getRodriguesVector(self):
        result = cv2.Rodrigues(self.rotation)[0]
        return result

    def getRodVecAndTrans(self):
        result = np.zeros(6)
        result[3:6, 1] = self.getTranslation()
        result[:3, 1] = self.getRodriguesVector()
        return result
    ...
    def calcAngularDistance(self, h):
        ...
        trace = min(3.0, max(-1, trace))
        return 180*math.acos((trace - 1.0)/2.0)/math.pi


# a = np.array([[0.6, -0.8, 0], [0.8, 0.6, 0], [0, 0, 1]])
# b = np.zeros(3)
# c = np.eye(3)
# h1 = Hypothesis()
# h1.RotandTrans(a, b)
