...
class GlobalProperties:
    ...
    def __init__(self,
        ...
        self.config = config
        file = np.fromfile('/home/open/eth/2020spring/3DV/Project/C++/DSAC/7scenes/7scenes_chess/sensorTrans.dat',
                           dtype=float)
        self.sensorTrans = np.delete(file, 0).reshape(4, 4)
    # def getInstance() 不清楚这里是不是要判断是否有instance
    ...
    def getCamMat(self):
        centerX = self.imageWidth + self.xShift
        centerY = self.imageHeight + self.yShift
        f = self.focalLength
        camMat = np.array([
            [f, 0, centerX],
            [0, f, centerY],
            [0, 0, 1]
        ])             # 暂时先用numpy定义，源码是用opencv矩阵cv::Mat
        return camMat
